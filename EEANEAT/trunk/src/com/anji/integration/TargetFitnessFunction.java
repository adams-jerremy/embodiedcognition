...
public void setStimuli(double[][] s){ stimuli = s;}
public void setStimuli(List<List<Double>> l){ setStimuli(llToDA(l));}
public void setTarget(double[][] t){targets = t;}
public void setTarget(List<List<Double>> l){setTarget(llToDA(l));}
public void setStimuliAndTarget(Map<List<Double>,Double> m){
	double[][] s = new double[m.size()][];
	double[][] t = new double[m.size()][1];
	int oIdx = 0,iIdx;
	for(List<Double> k:m.keySet()){
		s[oIdx] = new double[k.size()];
		iIdx = 0;
		for(double d:k) s[oIdx][iIdx++] = d;
		t[oIdx++][0] = m.get(k);
	}
	setStimuli(s);
	setTarget(t);
}
public double[][] llToDA(List<List<Double>> l){
	double[][] da = new double[l.size()][l.get(0).size()];
	int o = 0, i;
	for(List<Double> x:l){
		i = 0;
		for(double d:x) da[o][i++]=d;
		++o;
	}
	return da;
}
