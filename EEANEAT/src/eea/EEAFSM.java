...
//It should be 50. 
public class EEAFSM{
	public static int NUM_STATES = 10, NUM_ACCEPTING = 2, INPUT_LENGTH = 10, 
		POPULATION_SIZE=10, NUM_POPS = 2, ESTIMATION_ITERATIONS = (int)((3.0/8.0)*NUM_POPS*POPULATION_SIZE),
		MUTATION_GENERATIONS = 50, PRINT_EVERY=1;
	public static final List<ExampleGenerator<Integer>> RUNS = new LinkedList<ExampleGenerator<Integer>>();
	public static ExampleGenerator<Integer> DEFAULT_EXAMPLE_GENERATOR = new RandomExample();
	public static final ListSet<Integer> ALPHABET = new ListSet<Integer>(Arrays.asList(0,1));
	public static final FSM<Integer> TARGET = FSM.randomFactory(NUM_STATES, NUM_ACCEPTING, ALPHABET);
	//private static final FSM<Integer> TARGET = FSM.simpleFactory2(2, ALPHABET);
	//private static final FSM<Integer> TARGET = FSM.badHandCodedFactory(ALPHABET);
	public static final Map<List<Integer>,Boolean> labelled = new HashMap<List<Integer>,Boolean>();
	public static final List<List<FSM<Integer>>> populations = new ArrayList<List<FSM<Integer>>>(NUM_POPS);
	public static final Random rand = new Random(System.nanoTime());
	public static final List<List<Integer>> ALL_INPUTS = generateAllInputs(INPUT_LENGTH);
	public static final Comparator<FSM<Integer>> fitnessComp = new Comparator<FSM<Integer>>(){
		public int compare(FSM<Integer> o1, FSM<Integer> o2){return o1.fitness()>o2.fitness()?-1:o2.fitness()>o1.fitness()?1:0;}};
	public static final Comparator<Pair<List<Integer>,Double>> sentComp = new Comparator<Pair<List<Integer>,Double>>(){
	...
	public static final class RandomExample implements ExampleGenerator<Integer>{
	...
	public static final class InOrderExample implements ExampleGenerator<Integer>{
	...
	public static final class ActiveExample implements ExampleGenerator<Integer>{
...
//						if(accuracy>.95 && accuracy<1.0){System.err.println("Approximation found, stored in file approx.fsm");
//							pop.get(0).write("approx.fsm");
//							System.exit(0);
//						}
...
//	}
	...
	public static List<Integer> randomSentence(){
//		List<Integer> ret = new ArrayList<Integer>(INPUT_LENGTH);
//		for(int i = 0;i<INPUT_LENGTH;++i) ret.add(ALPHABET.choice());
//		return ret;
		return randomSentence(INPUT_LENGTH,ALPHABET);
	}
	public static <T> List<T>  randomSentence(int length, ListSet<T> alph){
		List<T> ret = new ArrayList<T>(length);
		for(int i = 0;i<length;++i) ret.add(alph.choice());
		return ret;
	}
