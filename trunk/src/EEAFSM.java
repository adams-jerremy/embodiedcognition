...
// -r ra
public class EEAFSM {
	private static int NUM_STATES = 10,
			NUM_ACCEPTING = 2,
			INPUT_LENGTH = 10,
			POPULATION_SIZE = 10,
			NUM_POPS = 2,
			ESTIMATION_ITERATIONS = (int) ((3.0 / 8.0) * NUM_POPS * POPULATION_SIZE),
	...
	private static final ListSet<Integer> ALPHABET = new ListSet<Integer>(
			Arrays.asList(0, 1));
	// private static final FSM<Integer> TARGET = FSM.randomFactory(NUM_STATES,
	// NUM_ACCEPTING, ALPHABET);
	private static final FSM<Integer> TARGET = FSM
			.badHandCodedFactory(ALPHABET);
	private static final Map<List<Integer>, Boolean> labelled = new HashMap<List<Integer>, Boolean>();
	private static final List<List<FSM<Integer>>> populations = new ArrayList<List<FSM<Integer>>>(
			NUM_POPS);
	...
	private static final Comparator<FSM<Integer>> fitnessComp = new Comparator<FSM<Integer>>() {
		public int compare(FSM<Integer> o1, FSM<Integer> o2) {
			return o1.fitness() > o2.fitness() ? -1 : o2.fitness() > o1
					.fitness() ? 1 : 0;
		}
	};
	private static final Comparator<Pair<List<Integer>, Double>> sentComp = new Comparator<Pair<List<Integer>, Double>>() {
		public int compare(Pair<List<Integer>, Double> o1,
				Pair<List<Integer>, Double> o2) {
			return o1.second > o2.second ? -1 : o2.second > o1.second ? 1 : 0;
		}
	};

	private static final class RandomExample implements
			ExampleGenerator<Integer> {
	...
	private static final class InOrderExample implements
			ExampleGenerator<Integer> {
		...
		public List<Integer> generateExample() {
			if (!it.hasNext())
				it = generateAllInputs(INPUT_LENGTH).iterator();
			return it.next();
		}
	...
	private static final class ActiveExample implements
			ExampleGenerator<Integer> {
		...
		public List<Integer> generateExample() {
			...
			for (int i = 0; i < POPULATION_SIZE * NUM_POPS; ++i)
				sentencePop.add(new Pair<List<Integer>, Double>(
						randomSentence(), 0.0));
			for (int gen = 0; gen < MUTATION_GENERATIONS; ++gen) {
				...
				for (int i = 0; i < ESTIMATION_ITERATIONS; ++i) {
					int fit = rand.nextInt(sentencePop.size()), unfit = rand
							.nextInt(sentencePop.size());
					if (fit > unfit) {
						int temp = fit;
						fit = unfit;
						unfit = temp;
					}
					sentencePop.get(unfit).first = mutateSentence(sentencePop
							.get(fit).first);
				}
			}
			evaluateSentences(sentencePop);
	...
	private static void handleArgs(String[] args) {
		...
		for (Iterator<String> i = Arrays.asList(args).iterator(); i.hasNext();) {
			...
			case '-':
				switch (s.charAt(1)) {
				case 'r':
					setRuns(i.next().toLowerCase());
					break;
				case 'g':
					setGenerations(i.next());
					break;
				case 'p':
					setPopulationSize(i.next());
					break;
				case 's':
					setSize(i.next());
					break;
	...
	private static void setRuns(String s) {
		for (char c : s.toCharArray())
			switch (c) {
			case 'r':
				RUNS.add(RandomExample.only);
				break;
			case 'a':
				RUNS.add(ActiveExample.only);
				break;
			case 'i':
				RUNS.add(InOrderExample.only);
				break;
			default:
				System.err.println("Unrecognized Type Requested:" + s);
				break;
			}
	}
	...
	private static void setGenerations(String s) {
		try {
			MUTATION_GENERATIONS = Integer.parseInt(s);
		} catch (NumberFormatException e) {
			System.err.println("Imparseable number of generations: " + s);
		}
	}

	private static void setPopulationSize(String s) {
		try {
			POPULATION_SIZE = Integer.parseInt(s);
		} catch (NumberFormatException e) {
			System.err.println("Imparseable number for population: " + s);
		}
	}

	private static void setSize(String s) {
		try {
			NUM_STATES = Integer.parseInt(s);
		} catch (NumberFormatException e) {
			System.err.println("Imparseable size: " + s);
		}
	}
	...
	public static void main(String[] args) {
		handleArgs(args);
		if (RUNS.isEmpty())
			RUNS.add(DEFAULT_EXAMPLE_GENERATOR);
		boolean found;
			...
			for (int i = 0; i < 1001; ++i) {
				if (i % PRINT_EVERY == 0) {
					...
					for (List<FSM<Integer>> pop : populations) {
						double accuracy = pop.get(0).accuracy(TARGET,
								ALL_INPUTS);
						System.err.println("Max in population fitness: "
								+ pop.get(0).fitness());
						System.err.println("Max in population accuracy: "
								+ accuracy);
						if (accuracy == 1.0) {
	...
	private static void est() {
		if (populations.size() == 0) {
			for (int i = 0; i < NUM_POPS; ++i) {
				...
				for (int j = 0; j < POPULATION_SIZE; ++j)
					populations.get(i).add(
							FSM.randomFactory(NUM_STATES, NUM_ACCEPTING,
									ALPHABET));
			}
		} else {
			for (List<FSM<Integer>> pop : populations) {
				...
				while (li.hasNext()) {
					li.next();
					li.set(FSM.randomFactory(NUM_STATES, NUM_ACCEPTING,
							ALPHABET));
				}
			}
		}
		for (List<FSM<Integer>> pop : populations) {
			for (int gen = 0; gen < MUTATION_GENERATIONS; ++gen) {
				for (FSM<Integer> fsm : pop)
					fsm.evaluate(labelled);
				Collections.sort(pop, fitnessComp);
				for (int i = 0; i < ESTIMATION_ITERATIONS; ++i) {
					int fit = rand.nextInt(pop.size()), unfit = rand
							.nextInt(pop.size());
					if (fit > unfit) {
						int temp = fit;
						fit = unfit;
					...
					if (unfit == 0)
						unfit = pop.size() - 1;
					pop.set(unfit, EvoFSM.mutate(pop.get(fit)));// TODO: Change
																// mutation to
																// also mutate
																// accepting
																// states?
	...
	// List<Pair<List<Integer>,Double>> sentencePop = new
	// LinkedList<Pair<List<Integer>,Double>>();
	// for(int i = 0;i<POPULATION_SIZE*NUM_POPS;++i) sentencePop.add(new
	// Pair<List<Integer>,Double>(randomSentence(),0.0));
	// for(int gen = 0; gen<MUTATION_GENERATIONS;++gen){
	// evaluateSentences(sentencePop);
	// Collections.sort(sentencePop,sentComp);
	// for(int i =0;i<ESTIMATION_ITERATIONS;++i){
	// int fit = rand.nextInt(sentencePop.size()),unfit =
	// rand.nextInt(sentencePop.size());
	// if(fit>unfit){ int temp = fit; fit = unfit; unfit =
	// (temp==0)?sentencePop.size()-1:temp;}
	// sentencePop.get(unfit).first =
	// mutateSentence(sentencePop.get(fit).first);
	// }
	// }
	// evaluateSentences(sentencePop);
	// double max = -Double.MAX_VALUE;
	...
	private static void evaluateSentences(
			List<Pair<List<Integer>, Double>> sentencePop) {
		for (Pair<List<Integer>, Double> p : sentencePop) {
			for (List<FSM<Integer>> pop : populations)
				// for(FSM<Integer> fsm:pop) p.second+=fsm.offer(p.first)?1:0;
				// Only offer top two to Exploration?
				p.second += (pop.get(0).offer(p.first) ? 1 : 0)
						+ (pop.get(1).offer(p.first) ? 1 : 0);
...
interface ExampleGenerator<T> {
	List<T> generateExample();
}
